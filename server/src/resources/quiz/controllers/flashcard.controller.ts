...
class FlashcardController {
  ...
  private initializeRoutes() {
    ...
    this.router.get(`${this.path}`, authenticatedMiddleware, this.getFlashcard);

    this.router.get(`${this.path}/:id`, this.getFlashcardTopic);
  ...
  private postFlashcard = async (
    ...
    try {
      const {
        ...
        category,
        flashcards,
      }: { title: string; category: string; flashcards: Flashcard[] } =
        req.body;
  ...
  private getFlashcardTopic = async (
    ...
    try {
      ...
      const flashcard =
        await this.FlashcardService.getFlashcardTopic(flashcardId);
