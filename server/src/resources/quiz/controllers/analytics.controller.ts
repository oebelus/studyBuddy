...
class analyticsController {
  ...
  private initializeRoutes(): void {
    ...
    this.router.get(`${this.path}/difficulty`, this.getDifficultyStats);
    this.router.post(`${this.path}/mcq`, this.postMcqAttempts);
  }
  ...
  private getDifficultyStats = async (
    ...
    try {
      ...
      const difficultyStats =
        await this.AnalyticsService.getDifficultyStats(userId);
      res.status(200).json(difficultyStats);
    } catch (err) {
      next(
        new HttpException(
          400,
          err instanceof Error ? err.message : "Failed to get difficulty stats",
        ),
      );
    }
  };
