...
app.get('/api/questions', async (req, res) => {
    ...
    const frenchPrompt = `Lisez tout le texte. Générez autant de questions que possible avec 5 réponses exactement à choix multiple à partir de ce texte : ${text} pour les étudiants en médecine. Fournissez également les réponses séparément (1, 2, 3, 4, 5). Et n'envoyez que le JSON directement, sans texte d'introduction s'il vous plaît, pour que le challenge réussisse ! Votre réponse doit être dans le format suivant : {"questions": [{"id": 0, "question": "", "options": [], "answers": []}, ...]}`
